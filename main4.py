...
class Dot:
    ...
    def avoid_obstacle(self):
        ...
        if dist < obstacle_radius*COEFF_DIST + self.size*COEFF_DIST:
            ...
            # Introduce a small deviation to move away from the target slightly
            angle_to_target = math.atan2(self.target_y - self.y, self.target_x - self.x)
            angle_offset = math.pi / 4  # 45 degrees deviation (adjust as needed)
            
            # Determine the direction to deviate (left or right)
            if (angle_to_target - angle_away_from_obstacle) % (2 * math.pi) < math.pi:
                self.angle = angle_away_from_obstacle + angle_offset
            else:
                self.angle = angle_away_from_obstacle - angle_offset

            # Move the dot in the adjusted direction
